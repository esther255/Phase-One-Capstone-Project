...
public class CustomerDAOImpl implements CustomerDAO {
    ...
    public void save(Customer customer) throws Exception {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            pstmt.setString(4, customer.getPin());
            pstmt.executeUpdate();
    ...
    public Optional<Customer> findByPhoneNumber(String phoneNumber) throws Exception {
        String sql = "SELECT id, full_name, email, phone_number, pin, created_at, failed_attempts FROM customers WHERE phone_number = ?";
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public Optional<Customer> findByEmail(String email) throws Exception {
        String sql = "SELECT id, full_name, email, phone_number, pin, created_at, failed_attempts FROM customers WHERE email = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public Optional<Customer> findById(int id) throws Exception {
        String sql = "SELECT id, full_name, email, phone_number, pin, created_at FROM customers WHERE id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, id);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    @Override
    public void updatePin(int customerId, String newPin) throws Exception {
